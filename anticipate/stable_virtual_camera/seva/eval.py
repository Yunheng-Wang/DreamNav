...
def chunk_input_and_test(
    ...
    for chunk in chunks:
        input_inds = [int(img[1:]) for img in chunk if img.startswith("!")]
        input_sels = [chunk.index(img) for img in chunk if img.startswith("!")]
        test_inds = [int(img[1:]) for img in chunk if img.startswith(">")]
...
class GradioTrackedSampler(EulerEDMSampler):
    ...
    def __call__(  # type: ignore
        ...
        verbose: bool = True,
        global_pbar: Optional[gr.Progress] = None,
        **guider_kwargs,
