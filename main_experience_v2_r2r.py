...
if __name__ == "__main__": 
    ...
    # opennav_id = [s["episode_id"]for s in load_task(cfg, "opennav")]
    files = os.listdir("/media/dreams/yunhengwang/DreamNav/main_experience/")
    all_finished = [int(f.split("_")[1]) for f in files if "_" in f]

    task = load_task(cfg, task_type)

    record = {} # 记录每个任务的导航过程
    while(True):
        rand = random.randint(0,1830)
        print(rand)
        for sig_task in task:
            if (sig_task["episode_id"] in all_finished) or (rand != sig_task["episode_id"]):
                continue
