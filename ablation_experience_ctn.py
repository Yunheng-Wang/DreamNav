...
if __name__ == "__main__": 
    ...
    for sig_task in task:
        ...
        while(True):
            ...
            #         "user": """
            #     You are given 6 descriptions of previously executed trajectories. Each describes what the agent would see or encounter along that trajectory. You are also given a current sub-task that describes what the agent is expected to do or reach.
            ...
            #         "trajectory_4": "<explanation of why it is or isn't suitable>",
            #         "trajectory_5": "<explanation of why it is or isn't suitable>"
            #     }},
            #     "choose": <Select the trajectory ID from 0 to 5 that you believe is most suitable for completing the current sub-task.>
            #     }}
            #     """ 
            #     }
            # navigator_prompt_input['user'] = navigator_prompt_input['user'].format(imagine[nav_step][0][1], imagine[nav_step][1][1], imagine[nav_step][2][1], imagine[nav_step][3][1], imagine[nav_step][4][1],imagine[nav_step][5][1], subtasks[subtasks_index])
            ...
            #         if ("0" in choose) or ("1" in choose) or ("2" in choose) or ("3" in choose) or ("4" in choose) or ("5" in choose):
            ...
            # elif "4" in choose:
            #     st = (cfg["hyper_parameter"]["imagine_step"])*2 - 1
            #     selected_trajectory_image = visualize_trajectories(intrinsic, rgb , all_trajectory_local[:, 4:5, :st, :], save_path, "selected_egocentric_trajectory")
            #     history[nav_step][1] = selected_trajectory_image
            #     record = action_trajectory(sim, agent_cfg, cfg, all_trajectory_local[0][4][:st], save_path, record, sig_task, video=True)
            
            # else:
            #     st = (cfg["hyper_parameter"]["imagine_step"])*2 - 1
            #     selected_trajectory_image = visualize_trajectories(intrinsic, rgb , all_trajectory_local[:, 5:6, :st, :], save_path, "selected_egocentric_trajectory")
            #     history[nav_step][1] = selected_trajectory_image
            #     record = action_trajectory(sim, agent_cfg, cfg, all_trajectory_local[0][5][:st], save_path, record, sig_task, video=True)
            # rgb,depth,_, _ , _ = get_current_rgb_depth(sim, save_path, save=True)
            # accessible_area_mask = segmentation_mask(rgb, save_path)
